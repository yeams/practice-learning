...
```
docker run -p 3306:3306 --name mysql0522 \
-v /opt/mysql/conf:/etc/mysql \
<!-- -v /opt/mysql/logs:/var/log/mysql \ -->
-v /opt/mysql/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=learning \
-d mysql
```
进入mysql: <names> = mysql0522
    - 进入容器docker exec -it <names> bash 
        + 查看容器情况： df -h
        + 进入mysql：mysql -uroot -plearning
4. 在mysql中创建learning数据库，字符编码utf8mb4
    docker run --name mysql-0522 -e MYSQL_ROOT_PASSWORD=learning -d mysql
    docker exec -it mysql-0522 bash 
...
问题：新键的container 建立以后不能start，原因：没有/var/log/mysql文件
#### 任务二
...
保存改动后的image：docker commit   
建立目录映射的MySQL容器： [Docker搭建MySQL8.0](https://www.bbsmax.com/A/xl56w0e1dr/)    
...
logging掌握（formatters, handlers, Logging Level, TimedRotatingFileHandler, MailHandler, logging.exception）   
...
任务二
用django写api实现以下查询:
查询出某个商品ID的一段时间内（比如：2014-01-01 00:00:00:00到2014-12-01 00:00:00:00）的最高价格、最低价格、平均价格
查询出某个商品ID每月的销量，销量的峰值在哪个月，销量的低谷在哪个月
最后用 django + tastypie(restful api)  +postman可以查询上述接口
